...
module.exports = async function handler(req, res) {
  ...
  const assetMatchers = app === 'orange'
    ? ['orange', 'money']
    : ['business'];

  if (!['business', 'orange'].includes(app)) {
    return res.status(400).json({ ok: false, error: 'Application inconnue.' });
  }
  ...
  try {
    const response = await fetch(`https://api.github.com/repos/${REPO}/releases/latest`, {
      headers: {
        ...
        'User-Agent': 'SIRA-Manager-Update-Service'
      }
    });
    ...
    return res.status(200).json({
      ...
      mandatory: Boolean(release.prerelease === false && String(release.body || '').includes('[MANDATORY]')),
