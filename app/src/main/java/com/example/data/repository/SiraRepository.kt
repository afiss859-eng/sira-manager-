...
class SiraRepository(private val db: SiraDatabase) {
    ...
    val allProductQrCodes: Flow<List<ProductQrCode>> = db.productQrCodeDao().getAllQrCodes()
    ...
    suspend fun recordSale(sale: Sale, items: List<SaleItem>): Long = withContext(Dispatchers.IO) {
        ...
        for (item in items) db.productDao().adjustQuantity(item.productId, -item.quantity)
        if (sale.status == SaleStatus.PAYE) {
            ...
            if (channel != CashChannel.AUTRE) {
                db.cashDao().insertCashTransaction(CashTransaction(type = CashOpType.VENTE_ENCAISSEMENT, channel = channel, amount = sale.totalAmount, reference = "ENC-${sale.reference}", beneficiaryOrPayer = sale.customerName, description = "Vente ${sale.reference} encaissée"))
            } else if (sale.customerId != null) {
                val cust = db.customerDao().getCustomerById(sale.customerId)
                if (cust != null) db.customerDao().updateCustomer(cust.copy(creditBalance = cust.creditBalance + sale.totalAmount))
            }
        }
        saleId
    }
    ...
    suspend fun insertSupplier(supplier: Supplier): Long = withContext(Dispatchers.IO) { db.supplierDao().insertSupplier(supplier) }
    suspend fun updateSupplier(supplier: Supplier) = withContext(Dispatchers.IO) { db.supplierDao().updateSupplier(supplier) }
    suspend fun deleteSupplier(supplier: Supplier) = withContext(Dispatchers.IO) { db.supplierDao().deleteSupplier(supplier) }
    suspend fun recordCashTransaction(tx: CashTransaction): Long = withContext(Dispatchers.IO) { db.cashDao().insertCashTransaction(tx) }
    suspend fun recordSyncLog(log: SyncLog): Long = withContext(Dispatchers.IO) { db.syncDao().insertSyncLog(log) }
    ...
    suspend fun performGlobalSearch(query: String): GlobalSearchResult = withContext(Dispatchers.IO) {
        ...
        val products = allProducts.first().filter { it.name.lowercase().contains(q) || it.category.lowercase().contains(q) || (it.barcode?.lowercase()?.contains(q) == true) }
        val customers = allCustomers.first().filter { it.fullName.lowercase().contains(q) || it.phone.contains(q) || (it.idDocumentNumber?.lowercase()?.contains(q) == true) }
        val sales = allSales.first().filter { it.reference.lowercase().contains(q) || it.customerName.lowercase().contains(q) }
        val purchases = allPurchases.first().filter { it.reference.lowercase().contains(q) || it.supplierName.lowercase().contains(q) }
        val cashTx = allCashTransactions.first().filter { it.reference.lowercase().contains(q) || (it.beneficiaryOrPayer?.lowercase()?.contains(q) == true) || it.description.lowercase().contains(q) }
        GlobalSearchResult(products = products, customers = customers, sales = sales, purchases = purchases, cashTransactions = cashTx)
    }
}
