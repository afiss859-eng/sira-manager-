    ...
    MaterialTheme {
        Scaffold { padding ->
            LazyColumn(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) { Text("Commissions", style = MaterialTheme.typography.labelLarge); Text("$commissions FCFA", style = MaterialTheme.typography.headlineMedium) } } }
                item {
                    OutlinedTextField(value = amount, onValueChange = { amount = it.filter(Char::isDigit) }, label = { Text("Montant") }, modifier = Modifier.fillMaxWidth())
                }
                item {
                    Button(onClick = {
                ...
                items(transactions) { tx -> ListItem(headlineContent = { Text(tx.reference) }, supportingContent = { Text("${tx.amount} FCFA • commission ${tx.fee} FCFA") }) }
            }
        }
    }
}
